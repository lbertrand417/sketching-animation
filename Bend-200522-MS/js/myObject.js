...
*/
        ...
        let a = worldRotation.angle / this.effector
        ...
        //for(let i = 1; i < 2; i++) {
            ...
            //q.setFromAxisAngle(localAxis, a);
            a += worldRotation.angle / this.effector
        ...
        //for (let i = 0; i < this.lengthBones; i++) {
            ...
            //vector.setFromMatrixPosition(this.restBones[i].matrixWorld); 
    ...
    updateForces(force) {
        ...
        /*const u = this._simulation.positions[this._simulation.positions.length - 1].clone().sub(this._simulation.positions[0]);
        u.normalize();
        this._simulation.forces[this._simulation.forces.length - 1].add(u.clone().multiplyScalar(K * (this._simulation.positions[this._simulation.positions.length - 1].distanceTo(this._simulation.positions[0]) - (this.lengthLinks - 1) * L0)));*/
    }
    ...
    updateBones2(quaternions) {
        ...
        for(let i = 1; i < this.lengthBones - 1; i++) {
            //let q2 = this.bones[i].quaternion.clone();
            ...
            /*if (i < this.effector + 1) {
                this.bones[i].applyQuaternion(quaternions[i-1].clone().slerp(q, 0.5));
                //this.bones[i].applyQuaternion(q2.clone().slerp(q, 0));
            } else {
                this.bones[i].applyQuaternion(quaternions[i-1].clone().slerp(q, param));
                //this.bones[i].applyQuaternion(q2.clone().slerp(q, param));
            }*/
            this.bones[i].applyQuaternion(quaternions[i-1].clone().slerp(q, param));
    ...
    updateLinksDisplay() {
        ...
        for(let i = 0; i < this.lengthLinks; i++) {
            ...
            this.links[i].position.set(this._simulation.positions[i+1].x, this._simulation.positions[i+1].y, this._simulation.positions[i+1].z);
